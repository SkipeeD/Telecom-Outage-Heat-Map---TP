...
import { getTechnicians } from '@/lib/firestore'
...
interface Props {
  ...
  open: boolean
  onClose: () => void
  onSave: (incidentNumber: string, technicians: IncidentAssignee[]) => Promise<void>
}

/**
 * Dispatch modal used by engineers to send field technicians to an incident.
 * Mirrors AssignEngineersModal but is scoped to the technician pool and writes
 * the incident's `technicians` list.
 */
export function AssignTechniciansModal({ incident, open, onClose, onSave }: Props) {
  ...
  // Reset selection and load technicians when incident changes
  useEffect(() => {
    void (async () => {
      if (!open || !incident) {
        setTechLoaded(false)
  ...
  async function handleSave() {
    ...
    setSaveError(null)
    try {
      const next: IncidentAssignee[] = technicians
        .filter(t => pendingUids.has(t.uid))
        .map(t => ({ uid: t.uid, email: t.email, ...(t.displayName ? { displayName: t.displayName } : {}) }))
      await onSave(incident.incidentNumber, next)
      onClose()
    } catch (err) {
      setSaveError(err instanceof Error ? err.message : 'Failed to dispatch technicians')
    } finally {
      setSaving(false)
    }
  }
      ...
      {open && incident && (
        <>
          ...
          <motion.div
            ...
            role="dialog"
            aria-modal
            aria-label="Dispatch technicians"
          ...
          >
              ...
              <div className="flex items-start justify-between gap-3 mb-3.5">
                <div>
                  ...
                  <div className="text-[12px] text-[var(--text-muted)] mt-0.5">
                    Send field technicians to resolve this incident on-site
                  </div>
                </div>
                <button
                  onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 min-h-0">
              {loadingTech ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 gap-1.5 text-center">
                  <span className="text-[13px] text-[var(--text-secondary)]">No technicians found.</span>
                  <span className="text-[11px] text-[var(--text-muted)]">An admin must assign the technician role first.</span>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-2">
                  {technicians.map(tech => {
                    const checked  = pendingUids.has(tech.uid)
                    const label    = tech.displayName ?? tech.email.split('@')[0]
                    const initials = label.slice(0, 2).toUpperCase()
                    return (
                      <motion.label
                        key={tech.uid ?? tech.email}
                        whileTap={{ scale: 0.98 }}
                        ...
                        style={{
                          ...
                          background: checked ? 'var(--accent-dim)' : 'var(--glass-bg)',
                        }}
                      >
                        {/* Avatar */}
                        <div
                        ...
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={e => toggleUid(tech.uid, e.target.checked)}
                          className="sr-only"
                        />
            ...
            <div className="shrink-0 flex items-center justify-between gap-3 px-5 py-4 border-t border-[var(--glass-border)] bg-[var(--glass-bg)]">
              <div className="flex flex-col gap-1">
                <span className="font-mono text-[11px] text-[var(--text-muted)]">
                  {pendingUids.size === 0
                    ...
                    : `${pendingUids.size} technician${pendingUids.size !== 1 ? 's' : ''} selected`}
                </span>
                {saveError && (
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  size="sm"
                  disabled={saving || loadingTech || technicians.length === 0}
                  onClick={handleSave}
                  className="
                    h-8 px-4 text-[11px] font-medium uppercase tracking-widest
