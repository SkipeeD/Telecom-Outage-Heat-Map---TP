...
export default function Navbar() {
  ...
  useEffect(() => {
    const measure = () => {
      ...
      setCenterOffset((actionsW - brandW) / 2)
    }
    measure()
  ...
  return (
    ...
    ">
      ...
      {/* Pill — lives in the centre 1fr column (between brand and actions).
          On dashboard we shift it by half the (actions − brand) width delta so
          it lands at viewport-centre; on map the offset is 0 so it stays
          centred between siblings. CSS-transitions the transform alongside
          the filter strip's max-width morph. */}
      <div
        style={{
      ...
      >
        ...
        {/* Filter strip — pure-CSS morph. max-width clips the strip; the pill's
            flex layout shrinks naturally with it. negative margin-left collapses
            the parent gap so there's no orphan 6px when fully retracted. */}
        <div
          ...
          style={{
            ...
            transition:
              'max-width 360ms cubic-bezier(0.4, 0, 0.2, 1),' +
      ...
      <motion.div
        ...
        className="justify-self-end flex items-center gap-5"
      >
